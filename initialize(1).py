...
def time_checker(pulse_list):
    ...
    for i in range(len(pulse_list)-1):
        ...
        if (np.floor(end_time*0.384))/0.384 < pulse_list[i+1][2]  < (np.floor(end_time*0.384) +1)/0.384:
            result.append(True)
        elif pulse_list[i][3] < 3/0.384:
            result.append(True)
        else:
            result.append(False)
    ...
    def body(self):
        ...
        if len(pulse_list) > 0: 
            ...
            for i in range(1, len(pulse_list)):
                ...
                freqNZ = pulse_list[i][5]#self.freq2reg(pulse_list[0][5] ,gen_ch=res_ch, ro_ch=cfg["ro_chs"][0])
                if freqNZ < 3000:
                    self.declare_gen(ch=res_ch, nqz=1)
                else:
                    self.declare_gen(ch=res_ch, nqz=2)
                ...
                length= int(pulse_list[i][3]*0.384) #length of pulse in clock cycles;
                start_time_clock = int(np.floor(pulse_list[i][2]*0.384))
                #instead of typing in 2.6, shouldn't we use this function instead? self.cycles2us(1000, gen_ch=res_ch)
                #although I'm not sure why it's 1000 clock cycles instead of 1...@Khanh?
